...
<body>
	...
	<div class="container">
		<div style="text-align: center">
			...
			<h6>
				<b>Spring Data With Server Side Pagination <br>Default 5 per page. Using Datatable's "search" only. <br>All other datatable features are removed.</b>
			</h6>
			<div style="text-align: center">
		...
		<div class="inline-block itemsperpage dropdown">
			...
			<div class="form-group col-md-11 pagination-centered">
				<ul class="pagination">
					<li class="page-item"><a class="page-link"
						href="/userlist2?pageSize=${selectedPageSize}&page=1">First</a></li>

					<!--<li><a class="page-link" -->
					<%--href="/userlist2?pageSize=${selectedPageSize}&page=${users.number}"><i --%>
					<!--class="fas fa-arrow-left"></i></a></li> -->

					<c:choose>
						<c:when test="${users.number != 0}">
							<li><a class="page-link"
								href="/userlist2?pageSize=${selectedPageSize}&page=${users.number}"><i
									class="fas fa-arrow-left"></i></a></li>
						</c:when>
						<c:otherwise>
							<li><i class="page-link fas fa-arrow-left"
								style="color: gray;"></i></li>
						</c:otherwise>
					</c:choose>

					<c:forEach begin="1" end="${users.totalPages}" step="1"
						varStatus="tagStatus">
						<c:url
							value="/userlist2?pageSize=${selectedPageSize}&page=${tagStatus.index}"
							var="url" />
						<a class="page-link" href='<c:out value="${url}" />'>${tagStatus.index}</a>
					</c:forEach>

					<c:choose>
						<c:when test="${users.number+1 == users.totalPages}">
							<li><i class="page-link fas fa-arrow-right"
								style="color: gray;"></i></li>
						</c:when>
						<c:otherwise>
							<li><a class="page-link"
								href="/userlist2?pageSize=${selectedPageSize}&page=${users.number + 2}"><i
					...
					<li class="page-item"><a class="page-link"
						href="/userlist2?pageSize=${selectedPageSize}&page=${users.totalPages}">Last</a></li>
				</ul>
			</div>
		</div>
...
$(document).ready(function() {
	...
	function changePageAndSize() {
		$('#pageSizeSelect').change(
				function(evt) {
					window.location.replace("/userlist2?pageSize=" + this.value
							+ "&page=1");
				});
	}
...
// 						});
				...
				$('#tableitems').DataTable({"bPaginate": false, "bInfo" : false});
